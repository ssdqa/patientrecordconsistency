...
`prc_output` function which check you executed. Reference the table
...
First, execute the **Single Site, *Anomaly Detection*, Cross-Sectional**
...
through the `squba.gen` package. The `p_value` can be selected by the
user.
...
pass it into the `ms_anom_euclidean` function, available through the
`squba.gen` package.
