...
testdb <- NULL
my_directory <- system.file(package = 'patientrecordconsistency')
my_file_folder <- system.file('extdata', package = 'patientrecordconsistency')
...
mk_testdb_omop <- function(){
  ...
  for (file_name in list.files(path=system.file('extdata',
    ...
    tbl <- readr::read_csv(
      file = file.path(system.file('extdata', package = 'patientrecordconsistency'),
                       file_name),
