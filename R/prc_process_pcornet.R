...
prc_process_pcornet <- function(cohort,
  ...
  if(!time){
    ...
    if(multi_or_single_site == 'single' & anomaly_or_exploratory == 'anomaly'){
      ...
      prc_tbl_int <- compute_prc_ntanom_pcnt(cohort = cohort_prep,
                                             site_col = site_col,
      ...
      prc_tbl_jacc_int <- compute_prc_ntanom_pcnt(cohort = cohort_prep,
                                                  site_col = site_col,
                                                  grouped_list = grouped_list,
                                                  event_tbl = prc_event_file,
                                                  grp_breaks = fu_breaks)
      ...
      prc_tbl_int <- compute_event_counts_pcnt(cohort = cohort_prep,
                                               grouped_list = grouped_list,
                                               site_col = site_col,
                                               time = time,
                                               event_tbl = prc_event_file)
    ...
    prc_tbl <- compute_fot(cohort = cohort_prep,
                           ...
                           check_func = function(dat){
                             compute_event_counts_pcnt(cohort = dat,
                                                       grouped_list = grouped_list,
                                                       site_col = site_col,
                                                       time = time,
